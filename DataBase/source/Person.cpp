...
int CreatePerson(pqxx::transaction_base& txn, const std::string first_name,
                 const std::string last_name, const std::string tg_nick, int access, int snils) {
    ...
    try {
        ...
        sql
            = "INSERT INTO people (first_name, last_name, tg_nick, access , snils) VALUES ($1, $2, "
              "$3, $4 , $5) RETURNING person_id";
...
std::tuple<std::string, std::string, std::string, int, int> ReadPerson(pqxx::transaction_base& txn,
                                                                       int person_id) {
    try {
        std::string sql
            = "SELECT first_name, last_name, tg_nick, access , snils FROM people WHERE person_id = "
              "$1";
        ...
        std::string first_name
            = row["first_name"].is_null() ? "" : row["first_name"].as<std::string>();
        std::string last_name
            = row["last_name"].is_null() ? "" : row["last_name"].as<std::string>();
