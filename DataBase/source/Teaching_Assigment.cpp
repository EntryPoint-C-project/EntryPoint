...
int CreateTeachingAssigment(pqxx::transaction_base& txn, int person_id, int offer_id,
    ...
    try {
        std::string sql
            = "INSERT INTO Teaching_Assigment (person_id, offer_id, subject_id) VALUES ($1, $2, "
              "$3)  ReTURNING assignment_id";
...
std::tuple<int, int, int> ReadTeachingAssigment(pqxx::transaction_base& txn, int assignment_id) {
    try {
        std::string sql
            = "SELECT person_id, offer_id, subject_id FROM Teaching_Assigment WHERE assignment_id "
              "= $1";
...
std::vector<std::pair<int, int> > ReadInfoOnPersonId(pqxx::transaction_base& txn, int person_id) {
    try {
        std::string sql
            = "SELECT offer_id, subject_id FROM Teaching_Assigment WHERE person_id = $1";
...
std::vector<std::pair<int, int> > ReadInfoOnOfferIdNoStudents(pqxx::transaction_base& txn,
    ...
    try {
        std::string sql
            = "SELECT person_id, subject_id FROM Teaching_Assigment WHERE offer_id = $1 AND "
              "subject_id != $2";
...
void UpdateTeachingAssigment(pqxx::transaction_base& txn, int assignment_id, int person_id,
    ...
    try {
        std::string sql
            = "UPDATE Teaching_Assigment SET person_id = $1, offer_id = $2, subject_id = $3 WHERE "
              "assignment_id = $4";
...
bool IsThereARecordTeachingAssigment(pqxx::transaction_base& txn, int person_id, int offer_id,
    ...
    try {
        std::string sql
            = " SELECt EXISTS("
              "SELECT 1 FROM Teaching_Assigment WHERE person_id = $1 AND offer_id = $2 AND "
              "subject_id = $3"
              ")";
        pqxx::result res = txn.exec_params(sql, person_id, offer_id, subject_id);
...
int GetTeachingAssigmentId(pqxx::transaction_base& txn, int person_id, int offer_id,
    ...
    try {
        std::string sql
            = "SELECT assignment_id FROM Teaching_Assigment WHERE person_id = $1 AND offer_id = $2 "
              "AND subject_id = $3";
        pqxx::result res = txn.exec_params(sql, person_id, offer_id, subject_id);
        // //txn.commit();
