...
int CreateSubjectOffer(pqxx::transaction_base& txn, int people_group_id, int program_id,
                       int course_id) {
    ...
    try {
        std::string sql
            = "INSERT INTO subject_offer (people_group_id, program_id, course_id) VALUES ($1, $2, "
              "$3)  ReTURNING offer_id";
        ...
        if (!res.empty()) {
            ...
            fmt::print(
                "Запись уже существует с указанным people_group_id, program_id, course_id\n");
        }
        // txn.commit() ;
        return offer_id;
...
std::tuple<int, int, int> ReadSubjectOffer(pqxx::transaction_base& txn, int offer_id) {
    try {
        std::string sql
            = "SELECT people_group_id, program_id, course_id FROM subject_offer WHERE offer_id = "
              "$1";
...
void UpdateSubjectOffer(pqxx::transaction_base& txn, int offer_id, int people_group_id,
                        int program_id, int course_id) {
    try {
        std::string sql
            = "UPDATE subject_offer SET people_group_id = $1, program_id = $2, course_id = $3 "
              "WHERE offer_id = $4";
...
bool IsThereARecordOffer(pqxx::transaction_base& txn, int people_group_id, int program_id,
                         int course_id) {
    try {
        std::string sql
            = " SELECt EXISTS("
              "SELECT 1 FROM subject_offer WHERE people_group_id = $1 AND program_id = $2 AND "
              "course_id = $3"
              ")";
        pqxx::result res = txn.exec_params(sql, people_group_id, program_id, course_id);
...
int GetOfferId(pqxx::transaction_base& txn, int people_group_id, int program_id, int course_id) {
    try {
        std::string sql
            = " SELECt offer_id FROM subject_offer WHERE people_group_id = $1 AND program_id = $2 "
              "AND course_id = $3";
        pqxx::result res = txn.exec_params(sql, people_group_id, program_id, course_id);
