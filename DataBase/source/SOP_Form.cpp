...
int CreateSOPForm(pqxx::transaction_base& txn, int person_id, const std::string url,
                  std::string tg_answer, std::string url_answer) {
    try {
        std::string sql
            = "INSERT INTO SOP_Form (person_id, url_out_sop, sop_status ,  tg_answer, url_answer) "
              "VALUES ($1, $2, $3, $4, $5)  RETURNING sop_id";
        pqxx::result res
            = txn.exec_params(sql, person_id, url, "NOT_STARTED", tg_answer, url_answer);
        int sop_id;
        if (!res.empty()) {
...
std::tuple<int, std::string, std::string, std::string, std::string> ReadSOPForm(
    pqxx::transaction_base& txn, int sop_id) {
    try {
        std::string sql
            = "SELECT person_id, url_out_sop, sop_status ,  tg_answer, url_answer FROM SOP_Form "
              "WHERE sop_id = $1";
...
void UpdateTgAnswer(
    pqxx::transaction_base& txn, int person_id,
    std::string new_tg_answer) {  // обновленные методы , для того , чтобы у нас была конкатенация
                                  // ответов , а не просто замена одного на другое
    try {
        std::string sql
            = "UPDATE SOP_Form SET tg_answer = tg_answer || '\t' || $1 WHERE person_id = $2";
...
void UpdateUrlAnswer(
    pqxx::transaction_base& txn, int person_id,
    std::string new_url_answer) {  // обновленные методы , для того , чтобы у нас была конкатенация
                                   // ответов , а не просто замена одного на другое
    try {
        std::string sql
            = "UPDATE SOP_Form SET url_answer = url_answer || '\t' || $1 WHERE person_id = $2";
