...
int CreatePeopleGroup(pqxx::transaction_base &txn, const std::string &people_group_name) {
    ...
    try {
        std::string sql
            = "INSERT INTO people_group (people_group_name) VALUES ($1)  ReTURNING people_group_id";
...
void UpdatePeopleGroup(pqxx::transaction_base &txn, int people_group_id,
                       std::string new_people_group_name) {
    try {
        std::string sql
            = "UPDATE people_group SET people_group_name = $1 WHERE people_group_id = $2";
