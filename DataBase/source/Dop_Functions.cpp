...
std::vector<Teacher> GetAllTeachersForStudent(pqxx::transaction_base& txn, int student_id) {
    ...
    for (auto param : student_params) {
        ...
        if (subject_name == "Student") {
            std::tuple<int, int, int> subject_offer
                = ReadSubjectOffer(txn, offer_id);  // people_group_id, program_id, course_id
            std::vector<std::pair<int, int> > teacher_info
                = ReadInfoOnOfferIdNoStudents(txn, offer_id);
            for (auto info : teacher_info) {  // person_id , subject_id
                ...
                std::tuple<std::string, std::string, std::string, int, int> teacher_info
                    = ReadPerson(txn, person_id);
...
>>>>>>> last_branch
        ...
        if (IsThereARecordTeachingAssigment(txn, person_id, offer_id, subject_id)) {
            int teaching_assigment_id
                = GetTeachingAssigmentId(txn, person_id, offer_id, subject_id);
        } else {
            int teaching_assigment_id
