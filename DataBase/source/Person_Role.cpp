...
std::pair<int, int> CreatePersonRole(pqxx::transaction_base& txn, int person_id, int role_id) {
    ...
    try {
        std::string sql
            = "INSERT INTO person_role (person_id, role_id) VALUES ($1, $2) ON CONFLICT DO NOTHING";
...
void UpdatePersonRole(pqxx::transaction_base& txn, int person_id, int role_id, int new_person_id,
                      int new_role_id) {
    try {
        std::string sql
            = "UPDATE Person_Role SET person_id = $1, role_id = $2 WHERE person_id = $3 AND "
              "role_id = $4";
