...
public:
    SOP_Form(int person_id, std::string url, std::string tg_answer, std::string url_answer)
        : person_id(person_id), url(url), tg_answer(tg_answer), url_answer(url_answer) {}
...
void UpdateTgAnswer(pqxx::transaction_base& txn, int person_id, std::string new_tg_answer);
...
void GetUrl(pqxx::transaction_base& txn, std::string tg_nick);