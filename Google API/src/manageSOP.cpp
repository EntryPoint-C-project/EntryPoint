...
std::string HttpClient::performHttpRequest(const std::string &url, const std::string &method,
...
>>>>>>> last_branch
    ...
    for (const auto &teacher : teachers) {
        const std::string &teacherName = teacher.first_name + " " + teacher.last_name;
        ...
        try {
            ...
            overallQ3["createItem"]["item"]["title"]
                = "Есть ли у вас какие-нибудь пожелания по поводу преподавания " + teacherName
                  + "?";
    ...
    for (auto &request : form["requests"]) {
        try {
            ...
            Logger::getInstance().error("Error setting index for form request: ", e.what());
        }
    }
...
std::string refreshAccessToken(Config &config, HttpClient &httpClient) {
    ...
    if (std::chrono::duration_cast<std::chrono::seconds>(now - config.getLastUpdateTime())
        < config.getTokenExpirationTime()) {
        return config.getAccessToken();
    }
    std::string postData = "grant_type=refresh_token&client_id=" + config.getClientId()
                           + "&client_secret=" + config.getClientSecret()
                           + "&refresh_token=" + config.getRefreshToken();
    std::string response = httpClient.performHttpRequest("https://oauth2.googleapis.com/token",
                                                         "POST", "", postData);
    ...
    try {
        ...
        if (jsonResponse.contains("access_token")) {
            ...
            Logger::getInstance().error("Error: 'access_token' not found in response");
            if (jsonResponse.contains("error")) {
                Logger::getInstance().info("Error details: ", jsonResponse["error"].dump());
            }
            return "";
        }
    } catch (const std::exception &e) {
...
std::string createForm(const std::string &jsonFilePath, Config &config, HttpClient &httpClient) {
    ...
    if (accessToken.empty()) {
        Logger::getInstance().error("Failed to refresh access token while creating form");
        return "";
    }

    std::string formTitle = readJsonFromFile(jsonFilePath).dump();
    std::string response = httpClient.performHttpRequest("https://forms.googleapis.com/v1/forms",
                                                         "POST", accessToken, formTitle);
...
void deleteForm(const std::string &formId, Config &config, HttpClient &httpClient) {
    std::string accessToken = refreshAccessToken(config, httpClient);
    if (accessToken.empty()) {
        Logger::getInstance().error("Failed to refresh access token while delete form");
        return;
    }

    std::string url = "https://forms.googleapis.com/v1/forms/" + formId;
    std::string response = httpClient.performHttpRequest(url, "DELETE", accessToken);
}

json getFormResponses(const std::string &formId, Config &config, HttpClient &httpClient) {
    std::string accessToken = refreshAccessToken(config, httpClient);
    if (accessToken.empty()) {
        Logger::getInstance().error("Failed to refresh access token while get form response");
        return json();
    }

    std::string url = "https://forms.googleapis.com/v1/forms/" + formId + "/responses";
    std::string response = httpClient.performHttpRequest(url, "GET", accessToken, "");
...
json readGoogleTable(const std::string &tableName, const std::string &range, Config &config,
    ...
    std::string accessToken = refreshAccessToken(config, httpClient);
    if (accessToken.empty()) {
        Logger::getInstance().error("Failed to refresh access token while read Google table");
        return json();
    }

    std::string url = "https://sheets.googleapis.com/v4/spreadsheets/" + tableName
