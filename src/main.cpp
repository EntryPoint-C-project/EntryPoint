...
void student_call_back_query(TgBot::Bot &bot, TgBot::CallbackQuery::Ptr &query, std::shared_ptr<mtd::User> user) {
    ...
    if (query->data == "1" || query->data == "2" || query->data == "3" || query->data == "4" || 
        ...
        // Следующий вопрос
        if (user->GetStep() < 5) {
            bot.getApi().sendMessage(chat_id, "Вопрос " + std::to_string(user->GetStep() + 1), 0, 0, get_raiting_scale());
            user->GetStep()++; // Переход к следующему вопросу
        } else {
            bot.getApi().sendMessage(chat_id, "Опрос завершен. Спасибо за участие!", 0, 0, user->BackButton());
    ...
    if (query->data == "student_buttons") {
        bot.getApi().sendMessage(chat_id, "fsfsdf", 0, 0, user->GetInlineKeyboard());
    }
        ...
        bot.getApi().sendMessage(chat_id, "Ссылка на расписание", 0, 0, user->GetInlineKeyboard());
    }
    else if (query->data == "student_declaration") {
        bot.getApi().sendMessage(chat_id, "Актуальные объявления", 0, 0, user->GetInlineKeyboard());
    }
    else if (query->data == "student_connect_with_teacher") {
        bot.getApi().sendMessage(chat_id, "Списочек с контактами преподавателя", 0, 0, user->GetInlineKeyboard());
    }
    else if (query->data == "student_help") {
        bot.getApi().sendMessage(chat_id, "Помощь", 0, 0, user->GetInlineKeyboard());
    }
    else if (query->data == "student_back") {
        bot.getApi().sendMessage(chat_id, "Меню", 0, 0, user->GetMenu());
    }
    else if (query->data == "student_information") {
        bot.getApi().sendMessage(chat_id, "Какая-то информация", 0, 0, user->BackButton());
    }
}
...
void office_staff_call_back_query(TgBot::Bot &bot, TgBot::CallbackQuery::Ptr &query, std::shared_ptr<mtd::User> user) {
    ...
    if (query->data == "office_staff_buttons") {
        bot.getApi().sendMessage(chat_id, "Кнопочки", 0, 0, user->GetInlineKeyboard());
    }
    else if (query->data == "office_staff_time_table") {
        bot.getApi().sendMessage(chat_id, "Ссылка на расписание для office staff", 0, 0, user->GetInlineKeyboard());
    }
    else if (query->data == "office_staff_declaration") {
        bot.getApi().sendMessage(chat_id, "Актуальные объявления для office staff", 0, 0, user->GetInlineKeyboard());
    }
    else if (query->data == "office_staff_connect_with_teacher") {
        bot.getApi().sendMessage(chat_id, "Списочек с контактами преподавателя для office staff", 0, 0, user->GetInlineKeyboard());
    }
    else if (query->data == "office_staff_help") {
        bot.getApi().sendMessage(chat_id, "Помощь для office staff", 0, 0, user->GetInlineKeyboard());
    }
    else if (query->data == "office_staff_back") {
        bot.getApi().sendMessage(chat_id, "Меню для office staff", 0, 0, user->GetMenu());
    }
    else if (query->data == "office_staff_information") {
        bot.getApi().sendMessage(chat_id, "Какая-то информация для office staff", 0, 0, user->BackButton());
    }
    else if (query->data == "office_add_info") {
        bot.getApi().sendMessage(chat_id, "Здесь вы можете ввести информацию", 0, 0, user->GetInlineKeyboard());
    }
}
...
void teacher_call_back_query(TgBot::Bot &bot, TgBot::CallbackQuery::Ptr &query, std::shared_ptr<mtd::User> user) {
    ...
    if (query->data == "teacher_declaration") {
        bot.getApi().sendMessage(chat_id, "Вы преподаватель и вы можите сделать объявление", 0, 0, user->BackButton());
    }
    else if (query->data == "teacher_back") {
        bot.getApi().sendMessage(chat_id, "Меню", 0, 0, user->GetMenu());
    }
    else if (query->data == "teacher_buttons") {
        bot.getApi().sendMessage(chat_id, "Кнопочки для преподов", 0, 0, user->GetInlineKeyboard());
    }
    else if (query->data == "teachert_information") {
        bot.getApi().sendMessage(chat_id, "Какая-то полезная инва для преподов", 0, 0, user->BackButton());
    }
}
...
void tutor_call_back_query(TgBot::Bot &bot, TgBot::CallbackQuery::Ptr &query, std::shared_ptr<mtd::User> user) {
    ...
    if (query->data == "tutor_buttons") {
        bot.getApi().sendMessage(chat_id, "fsdf", 0, 0, user->GetInlineKeyboard());
    }
    else if (query->data == "tutor_add_subject") {
        bot.getApi().sendMessage(chat_id, "Введите название нового предмета", 0, 0, user->GetInlineKeyboard());
    }
        ...
        bot.getApi().sendMessage(chat_id, "Введите ID студентов и групп, которых нужно добавить", 0, 0, user->GetInlineKeyboard());
    }
    else if (query->data == "tutor_create_sop") {
        bot.getApi().sendMessage(chat_id, "Введите данные для создания SOP", 0, 0, user->GetInlineKeyboard());
    }
...
int main() {
    ...
    bot.getEvents().onCallbackQuery([&bot, &users, &mutex_for_users, &new_users](TgBot::CallbackQuery::Ptr query) {
        ...
        if (new_users.find(chat_id) != new_users.end()) {
            ...
            bot.getApi().sendMessage(chat_id, "Меню", 0, 0, users[chat_id]->GetMenu());
    ...
    bot.getEvents().onAnyMessage([&bot, &users, &mutex_for_users](TgBot::Message::Ptr message){
        ...
        if (user->GetState() == mtd::UserState::TUTOR_SOP) {
            ...
            std::string s;
            for (const auto i : users[student_chat_id]->GetEvaluations()) {
                s += std::to_string(i) + " ";
            }
            bot.getApi().sendMessage(chat_id, "Оценки этого студента:\n" + s, 0, 0, user->BackButton());
            return;
        }
    });
    try {
